...
export function createCameraRig(camera, domElement) {
  ...
  // EC-15: respect prefers-reduced-motion. When set, disable auto-rotate entirely
  // (it can be uncomfortable / trigger vestibular symptoms). We re-evaluate on
  // every change event so a live toggle takes effect without reload.
  const motionMql = (typeof window !== 'undefined' && window.matchMedia)
  ...
  if (motionMql.addEventListener) motionMql.addEventListener('change', applyMotionPreference)
  else if (motionMql.addListener) motionMql.addListener(applyMotionPreference)
  ...
  function update(dt, t) {
    // 静置 5 秒后星空开始极慢自转（3 秒渐入渐出）。EC-15: 如果用户偏好
    // 减少动效则完全跳过自转（不需要渐入再渐出）。
    const idle = t - lastInteraction
    const targetAuto = motionMql.matches ? 0 : (idle > 5 ? 1 : 0)
