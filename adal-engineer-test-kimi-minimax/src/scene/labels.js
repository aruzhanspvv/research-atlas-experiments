...
import { onLanguageChange, starName } from '../i18n.js'

// dustName stub: in the original atlas this localised micro-star labels.
// Dust stars were removed with the physics-history content; we keep the
// reference so any stray .dust flag in labelSource.js still renders.
const dustName = (d) => d?.name ?? `${d?.branch ?? 'dust'} waypoint · ${d?.year ?? ''}`
...
export function createLabels(stars, container) {
  ...
  // 优先级：影响力高者优先占位；ideas 排在中段——它们比 dust/low 重要，但让 influence-5 的高优先
  const byPriority = [...items].sort((a, b) => {
    if (a.tier === 'idea' && b.tier !== 'idea') return -1
    if (a.tier !== 'idea' && b.tier === 'idea') return 1
  ...
  function resolveOverlaps(radius) {
    ...
    byPriority.forEach((item) => {
      ...
      const isIdea = item.tier === 'idea'
      // 超密兜底：低层占位已达上限则直接压掉（单星聚焦不受限，流派聚焦仍受限）
      // ideas 永远不被超密兜底压掉——它们本来数量就少，且是核心交互对象
      if (!isIdea && isLow && lowShown >= LOW_TIER_CAP && !(focused && forceFocusedLabels)) {
