...
export function createLensAxes() {
  ...
  function rebuildAxes() {
    ...
    // —— Timeline —— x = (yearFrac - 2025.5) * 800; corpus spans roughly late 2025 to mid-2026
    const YEAR0 = 2025.5
    const SCALE_T = 800
    const tx = (yearFrac) => (yearFrac - YEAR0) * SCALE_T
    const yearTicks = [
      { yf: 2025.75, label: "Q4'25" },
      { yf: 2026.0, label: "Q1'26" },
      { yf: 2026.25, label: "Q2'26" },
      { yf: 2026.5, label: "Q3'26" }
    ]
    timeline = buildAxis({
      x0: tx(2025.6),
      x1: tx(2026.6),
      ticks: yearTicks.map((tk) => ({ x: tx(tk.yf), label: tk.label })),
    ...
    // —— Novelty —— x = noveltyExp*55; papers cluster near 0 (existing work),
    // generated ideas fan out toward 10 (frontier)
    const SX = 55
    const noveltyTicks = [
      { exp: 0, key: 'existing' },
      { exp: 2, key: 'incremental' },
      { exp: 4, key: 'notable' },
      { exp: 6, key: 'novel' },
      { exp: 8, key: 'breakthrough' },
      { exp: 10, key: 'frontier' }
    ].map((tick) => ({
      x: tick.exp * SX,
  ...
  function update(lensCurrent, lensAnim, dt) {
    ...
    const wantScale = targetLens === 'novelty' && animPass ? 1 : 0
