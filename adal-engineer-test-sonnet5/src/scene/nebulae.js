...
// topicMeta: Map from topics.js computeTopics() — {key, color, anchor, nebula} per topic.
// Replaces the old hardcoded BRANCHES import; nebula clouds now form
// dynamically around whatever topic clusters exist in the current dataset.
export function createNebulae(topicMeta) {
  const rand = mulberry32(9200)
  const clouds = [...topicMeta.values()].map((topic) =>
    buildBranchCloud(topic.key, topic, rand)
  )
