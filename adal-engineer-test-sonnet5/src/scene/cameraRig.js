...
export function createCameraRig(camera, domElement) {
  ...
  // Bug A fix: the galaxy overview previously always flew to a STATIC
  // hardcoded target/radius (OVERVIEW_TARGET/OVERVIEW_RADIUS), regardless
  // of how far the actual topic-cluster anchors (src/data/topics.js's
  // RING_RADIUS-based layout) spread from origin. Once topics.js pushed
  // clusters out to a ~980-unit ring (wider, on average, than the original
  // Physics-atlas's hand-authored 486-1175-unit branch anchors), the fixed
  // 1900-unit overview radius no longer reliably framed every cluster —
  // confirmed empirically: at the default overview, 8/18 seeded papers
  // projected outside the camera's NDC bounds (off-screen), while all the
  // dense on-screen space collapsed toward the near-origin look-at target,
  // reading as "random dots in the center" with nothing else clickable.
  // Fix: accept an optional {center, radius} override (the data-fitted view
  // main.js's lensViewpoint('galaxy') already computes for the OTHER
  // lenses) so the galaxy overview scales to the actual dataset instead of
  // a value tuned for the old smaller/tighter physics-history layout.
  function resetOverview(view) {
    const target = view?.center ?? OVERVIEW_TARGET
    const overviewRadius = view?.radius ?? OVERVIEW_RADIUS
    flyToView(target, overviewRadius, OVERVIEW_DIRECTION)
  }
