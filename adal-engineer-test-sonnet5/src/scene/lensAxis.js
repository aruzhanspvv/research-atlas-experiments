...
export function createLensAxes() {
  ...
  function rebuildAxes() {
    ...
    // —— Timeline axis —— x = (year-2020)*3.2, publish-date axis
    const YEAR0 = 2020
    const SCALE_T = 3.2
    ...
    timeline = buildAxis({
      x0: tx(2020),
      x1: tx(2027),
      ticks: [2020, 2022, 2024, 2026].map((y) => ({ x: tx(y), label: String(y) })),
      endLabel: 'PUBLISH DATE',
      tickLen: 10,
      tickLabelGap: 2,
      titleGap: 22
    })

    // —— Scale axis —— x = metric*55: papers by influence/citation bucket,
    // ideas by novelty score (both fields were normalized into `metric` by
    // placement.mjs / fetchSeed.mjs onto the same 0-300-ish range).
    const SX = 55
    const scaleTicks = [
      { x: 0 * SX, label: 'LOW' },
      { x: 1 * SX, label: 'CITED' },
      { x: 3 * SX, label: 'INFLUENTIAL' },
      { x: 5 * SX, label: 'NOVEL IDEA' }
    ]
    scale = buildAxis({
      x0: -1 * SX,
      x1: 5.5 * SX,
      ticks: scaleTicks,
      endLabel: 'INFLUENCE / NOVELTY'
    })
