...
| [`adal-engineer-test-kimi-minimax/`](./adal-engineer-test-kimi-minimax) | Adal engineer test run (Kimi K3 as main engineer, MiniMax M3 as builder/evaluator workers) | Static frontend with a single client-side LLM module (`src/lib/llm.js`) gating all three idea-generation entry points (topic / pasted arXiv refs / seed corpus); novelty is encoded on the idea-star itself (ring radius, dash density, hue, pulse) rather than only in the detail card. |
...
Note: `claude-code/` and `claude-code-goal/` are fully static with data baked
in at build time (no LLM calls at runtime). `adal-engineer-test-sonnet5/`
needs its Express server running plus an LLM API key for idea generation to
work live. `adal-engineer-test-kimi-minimax/` also needs an LLM API key
(client-side, see its `.env.local.example`) for the Ideate panel, but has no
